(function game() {
  ...
  function check4win(mark) {
    ...
    if (!computerLogic.isMovesLeft(playerX, playerO)) {
      alertWin("It's a tie!");
      gameended = true;
    }
  }
  function switchComputer(e) {
    if (e.target.classList.contains("human")) {
      if (e.target.parentNode.id === "playerX") {
        ...
        document.getElementById("buttons1").style.opacity = "1";
        document.getElementById("buttons1").style.pointerEvents = "unset";
      } else {
        OcomputerPlayer = !OcomputerPlayer;
        document.getElementById("buttons2").style.opacity = "1";
        document.getElementById("buttons2").style.pointerEvents = "unset";
      }
      e.target.classList.remove("human");
      e.target.classList.add("comp");
      e.target.innerText = "Computer";
    } else {
      if (e.target.classList.contains("comp")) {
        if (e.target.parentNode.id === "playerX") {
          XcomputerPlayer = !XcomputerPlayer;
          document.getElementById("buttons1").style.opacity = "0";
          document.getElementById("buttons1").style.pointerEvents = "none";
        } else {
          OcomputerPlayer = !OcomputerPlayer;
          document.getElementById("buttons2").style.opacity = "0";
          document.getElementById("buttons2").style.pointerEvents = "none";
        }
    ...
    if (XcomputerPlayer) {
      if (b1.checked) {
        if (oddTurn) {
          setNoClickClass();
          tout = setTimeout(() => {
            selectTile(computerLogic.findBestMove(playerX, playerO));
          }, 800);
          return;
        }
      } else {
        if (oddTurn) {
          setNoClickClass();
          tout = setTimeout(() => {
            selectTile(computerLogic.findRandomMove(playerX, playerO));
          }, 800);
          return;
        }
      }
      ...
      if (!oddTurn) {
        if (b2.checked) {
          if (!oddTurn) {
            setNoClickClass();
            tout = setTimeout(() => {
              selectTile(computerLogic.findBestMove(playerO, playerX));
            }, 800);
          }
        } else {
          if (!oddTurn) {
  ...
  function alertWin(message) {
    ...
    document.getElementById("ok").addEventListener("click", newGame);
    function newGame(e) {
      if (e.type == "keydown") {
        if (e.key == "Enter") {
...
const computerLogic = (function () {
  ...
  return { isMovesLeft, findBestMove, findRandomMove };
})();
...
function check(arr) {
  ...
  function check3(a) {
    if (
      comparesorted(a, [1, 2, 3]) ||
      comparesorted(a, [4, 5, 6]) ||
      comparesorted(a, [7, 8, 9]) ||
      comparesorted(a, [1, 4, 7]) ||
      comparesorted(a, [2, 5, 8]) ||
      comparesorted(a, [3, 6, 9]) ||
      comparesorted(a, [1, 5, 9]) ||
      comparesorted(a, [3, 5, 7])
    ) {
      return true;
    } else return false;
  }
